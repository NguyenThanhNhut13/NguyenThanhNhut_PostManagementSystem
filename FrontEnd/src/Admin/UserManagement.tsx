        ...
        <div className="card">
            ...
            <table className="table table-hover mb-0">
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  <tr
                    key={user.id}
                    className={isDeleting === user.id ? "opacity-50" : ""}
                  >
                    <td>
                        ...
                        <div>
                          <div className="fw-bold">
                            ...
                            {user.id === currentUser?.id && (
                              <span className="badge bg-success ms-2">Bạn</span>
                            )}
                          </div>
                    ...
                    <td>
                      <button
                        ...
                        disabled={
                          user.id === currentUser?.id || isDeleting === user.id
                        }
                        title={
                          user.id === currentUser?.id
          ...
          <div className="card-footer bg-light">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <small className="text-muted">
                Quản trị viên:{" "}
                {users.filter((u) => u.role === "ROLE_ADMIN").length} | Người
                dùng: {users.filter((u) => u.role === "ROLE_USER").length}
              </small>
            </div>
          </div>
        </div>
